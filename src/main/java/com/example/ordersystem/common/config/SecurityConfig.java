...
//                spring security에서 cors 정책 지정
                ...
                .httpBasic(AbstractHttpConfigurer::disable) // http basic보안방식 비활성화
...
//                .authenticated() : 모든 요청에대해서 Authentication객체가 생성되기를 요구
    ...
    private CorsConfigurationSource corsConfiguration(){
        ...
        configuration.setAllowedOrigins(Arrays.asList("http://localhost:3000","https://www.gorani.store"));
        configuration.setAllowedMethods(Arrays.asList("*"));//모든HTTP 메서드 허용
        configuration.setAllowedHeaders(Arrays.asList("*"));//모든 헤더 허용
        configuration.setAllowCredentials(true);//자격 증명 허용(인증처리하겠다)
